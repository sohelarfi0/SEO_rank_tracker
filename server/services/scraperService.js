...
export async function scrapeUrl(url) {
    ...
    try {
        ...
        const scrapedData = await page.evaluate(()=>{
            const getMeta = (name)=>{
                const el = document.querySelector(`meta[name="${name}"]`) || document.querySelector(
                    `meta[property="${name}"]`);
            ...
            allLinks.forEach((link)=>{
                try{
                    ...
                    if(linkUrl.hostname === currentHost) internalLinnks++;
                    else externalLinks++;
            ...
            return{
                metaData : {title, description, canonical, robots, ogTitle, ogDescription, ogImage,
                    twitterCard, viewport, charset
                },
                headings,
                links: {internal: internalLinnks, external: externalLinks, total: allLinks.length},
